...
router.post("/", async (req, res) => {
  try {
    ...
    if (!userId || !algorithmName) {
      return res
        .status(400)
        .json({ error: "Missing required fields: userId and algorithmName" });
    }
    ...
    if (existingProgress) {
      ...
      const update = {
        ...
        visualizationCount: visualizationCount
          ? existingProgress.visualizationCount + 1
          : existingProgress.visualizationCount,
      ...
      await db
        ...
        .updateOne({ userId, algorithmName }, { $set: update });
