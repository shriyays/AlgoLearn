...
class QuizPage {
  constructor() {
    ...
    this.algorithmName = new URLSearchParams(window.location.search).get(
      "name",
    );
    this.questions = [];
    this.userAnswers = [];
  ...
  getQuizQuestions(algorithmName) {
    const questionBank = {
      "Bubble Sort": [
        ...
        {
          question:
            "How many passes does Bubble Sort make in the worst case for an array of n elements?",
  ...
  setupEventListeners() {
    ...
    document.querySelectorAll(".option-label").forEach((label) => {
      label.addEventListener("click", () => {
        ...
        document
          .querySelectorAll(`.option-label[data-question="${questionIndex}"]`)
          .forEach((l) => {
  ...
  submitQuiz() {
    ...
    this.questions.forEach((q, index) => {
      ...
      const labels = document.querySelectorAll(
        `.option-label[data-question="${index}"]`,
      );
  ...
  async saveQuizResult(result) {
    try {
      ...
      console.warn(
        "⚠️ Backend unavailable, using localStorage:",
        error.message,
      );
  ...
  async updateProgressLocal(result) {
    ...
    const existing = progress.find(
      (p) => p.algorithmName === this.algorithmName,
    );
