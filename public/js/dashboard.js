...
class Dashboard {
  ...
  async loadData() {
    try {
      ...
      this.progress = this.algorithms.map((algo) => {
        const algoProgress = serverProgress.find(
          (p) => p.algorithmName === algo.name,
        );
        const algoQuizzes = this.quizResults.filter(
          (q) => q.algorithmName === algo.name,
        );
      ...
      this.streakData = JSON.parse(
        localStorage.getItem(streakKey) || '{"days": [], "currentStreak": 0}',
      );
      this.updateStreak();
    } catch (error) {
      console.error("Error loading dashboard data:", error);
      // Fallback to localStorage if backend fails
      this.quizResults = JSON.parse(
    ...
    if (!this.streakData.days.includes(today) && this.quizResults.length > 0) {
      ...
      for (let day of sortedDays) {
        ...
        const diffDays = Math.floor(
          (currentDate - dayDate) / (1000 * 60 * 60 * 24),
        );
  ...
  renderStats() {
    ...
    const avgScore =
      totalQuizzes > 0
        ? Math.round(
            this.quizResults.reduce((sum, q) => sum + q.percentage, 0) /
              totalQuizzes,
          )
        : 0;
  ...
  renderAchievements() {
    ...
    allAchievements.forEach((achievement) => {
      const savedAchievement = this.achievements.find(
        (a) => a.achievementType === achievement.id,
      );
      const unlockedDate = savedAchievement
        ? new Date(savedAchievement.unlockedAt).toLocaleDateString()
        : null;
