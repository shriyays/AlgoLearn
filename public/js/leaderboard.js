...
class Leaderboard {
  ...
  async loadLeaderboardData() {
    try {
      ...
      const response = await fetch(
        `/api/leaderboard?limit=10&sortBy=${this.currentSort}`,
      );
      this.leaderboardData = await response.json();

      // Get current user's rank
      const rankResponse = await fetch(
        `/api/leaderboard/${this.userId}/rank?sortBy=${this.currentSort}`,
      );
  ...
  calculateCurrentUserRank() {
    const sortedData = this.sortLeaderboard(
    ...
    return [...data].sort((a, b) => {
      switch (sortBy) {
        case "algorithmsCompleted":
          return (
            b.algorithmsCompleted - a.algorithmsCompleted ||
